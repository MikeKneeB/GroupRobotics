...
def train(sess, actor_model, critic_model, env, state_dim, action_dim, max_action, epochs = 1000, run_length = 300, batch_size = 40, gamma = 0.95, epsilon = 1, min_epsilon = 0.01, buffer = 1000, filepath=None, render=False):
    ...
    with open(filepath, 'w') as f:
        ...
        for epo in range(epochs):
            ...
            obs_1 = np.array([np.sin(obs_1[0]), np.cos(obs_1[0]), obs_1[1]])
            obs_1 = obs_1.reshape(state_dim, 1)
            ...
            for j in range(run_length):
                ...
                obs_2 = [obs_2[0][0][0], obs_2[1][0][0]]
                obs_2 = np.array([np.sin(obs_2[0]), np.cos(obs_2[0]), obs_2[1]])
                obs_2 = obs_2.reshape(state_dim, 1)
