...
def get_observations(action):
    torque = numpy.array([action, 0])
    observations = env.step(torque)
    positions, reward, failed, info = observations

    pendulumX, pendulumY, pendulumZ = positions
    angleRad = numpy.arctan(pendulumY/pendulumX)
    angleDeg = int(numpy.round(np.rad2deg(angleRad)*2))
    velocity = int(numpy.round(pendulumZ))
    reward = int(numpy.round(reward))
