//Controller Used to implement analytical motion
//Author: Daniel Butters 2016
...
*/
static double maxHip = 0.28;
static double minHip = -1.33;
static double maxKnee = 1.9;
static double minKnee = -0.05;
int legsToMax = 0;
int legsToMin = 0;
int bodyToMax = 0;
int bodyToMin = 0;
...
static double calcAccWithValues(double lthet1,double lthet2,double lthet3,double bthet1,double bthet2,double bthet3,double sthet1,double sthet2,double sthet3){
  double lthet[3] = {lthet1,lthet2,lthet3};
  double bthet[3] = {bthet1,bthet2,bthet3};
  double sthet[3] = {sthet1,sthet2,sthet3};
...
static double calcAcc(double kneeang1,double kneeang2,double bodyang1,double bodyang2,double swingang1,double swingang2){
  ...
  double AccT = calcAccWithValues( lthet[0], lthet[1], lthet[2],
                                   bthet[0], bthet[1], bthet[2],
                                   sthet[0], sthet[1], sthet[2]);
...
static double testAcc(double kneeang1,double kneeang2,double bodyang1,double bodyang2,double swingang1,double swingang2){
  ...
  for(int lcount = -1; lcount < 2; lcount = lcount + 1){
    ...
    for(int bcount = -1; bcount < 2; bcount++){
      ...
      AccT = calcAccWithValues( newlthet, lthet[1], lthet[2],
                                newbthet, bthet[1], bthet[2],
                                sthet[0], sthet[1], sthet[2]);
      if(AccT>greatestAcc){
        greatestAcc = AccT;
        bestlchange = lcount;
...
static double minmaxKneeAcc(bool direction, double kneeang1,double kneeang2,double bodyang1,double bodyang2,double swingang1,double swingang2){
  ...
  double AccTmax = calcAccWithValues( newlthetmax, lthet[1], lthet[2],
                                      bthet[0], bthet[1], bthet[2],
                                      sthet[0], sthet[1], sthet[2]);
  double AccTmin = calcAccWithValues( newlthetmin, lthet[1], lthet[2],
                                      bthet[0], bthet[1], bthet[2],
                                      sthet[0], sthet[1], sthet[2]);
...
static double minmaxBodyAcc(bool direction, double kneeang1,double kneeang2,double bodyang1,double bodyang2,double swingang1,double swingang2){
  ...
  double AccTmax = calcAccWithValues( lthet[0], lthet[1], lthet[2],
                                      newbthetmax, bthet[1], bthet[2],
                                      sthet[0], sthet[1], sthet[2]);
  double AccTmin = calcAccWithValues( lthet[0], lthet[1], lthet[2],
                                      newbthetmin, bthet[1], bthet[2],
                                      sthet[0], sthet[1], sthet[2]);
   double bestForwardAngle;
   double bestBackwardAngle;
   if(AccTmax>AccTmin){
...
static double bestAcc(bool direction, double kneeang1,double kneeang2,double bodyang1,double bodyang2,double swingang1,double swingang2){
  ...
  if(maxbcount == 2){
    double posR = wb_motor_get_target_position(RHipPitch);
    double targR = posR+angleChange*bestbchange;
    if(targR<minHip || targR > maxHip){
      minmaxBodyAcc(direction, kneeang1, kneeang2, bodyang1, bodyang2, swingang1, swingang2);
    }
    else{
  ...
  if(maxlcount == 2){
    double posR = wb_motor_get_target_position(RKneePitch);
    double targR = posR+angleChange*bestlchange;
    if(targR<minKnee || targR > maxKnee){
      minmaxKneeAcc(direction, kneeang1, kneeang2, bodyang1, bodyang2, swingang1, swingang2);
    }
    else{
...
int main(int argc, const char *argv[]) {
  ...
  while (1) {
    ...
    if(direction == 1 && lastDirection == 0){
      minAngle = swingang;
      printf("time = %f max = %f min = %f\n",time, maxAngle, minAngle);
    }
    if(direction == 0 & lastDirection == 1){
      maxAngle = swingang;
    }
    //printf("%d %d %d %d \n",legsToMax,legsToMin,bodyToMax,bodyToMin);
    if(count % 10 == 0){
        if(legsToMax){
          check_legs_max();
        }
