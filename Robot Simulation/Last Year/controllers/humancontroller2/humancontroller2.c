...
static bool check_swingang(double targAngle) {
  ...
  bool atTarget = 0;
  //printf("%f %f \n",seatangle[0], seatangle[1]);
  if(pitch<(targAngle+0.02) && pitch>(targAngle-0.02)){
    atTarget = 1;
  }
  return atTarget;
}
...
int main(int argc, const char *argv[]) {
  ...
  */
  ...
  /* with swing height = 200 and scaling
  double targLegForwards[12] = {0.595131701,0.071532925,-0.137906585,
  -0.24262634,-0.31243951,-0.347346095,
  -0.347346095,-0.31243951,-0.17281317,
  0.071532925,0.525318531,0.944197551};
  double targLegBackwards[12] = {0.595131701,1.328169987,1.747049007,
  1.921581932,1.886675347,1.781955592,
  1.677235837,1.642329252,1.642329252,
  1.607422667,1.397983157,0.944197551};
  */
  /* swing height = 155 but no swing offset
   double targLegForwards[12] = {0.595131701,0.071532925,-0.17281317,
   -0.24262634,-0.31243951,-0.347346095,
   -0.347346095,-0.277532925,-0.137906585,
   0.071532925,0.455505361,1.048917306};
  double targLegBackwards[12] = {0.595131701,1.363076572,1.747049007,
  1.886675347,1.886675347,1.781955592,
  1.712142422,1.677235837,1.677235837,
  1.642329252,1.432889742,1.048917306};
  */
  //with swing offset
  ...
  while (1) {
   ...
   /* bodyang1 = bodyang;
    ...
    if(direction == 1 && lastDirection == 0){
      ...
      for(int i = 0; i<12;i++){
        targAngles[i] = newAngles[i];
      }
    //  printf("0\n");
    }
    if(direction == 0 & lastDirection == 1){
      maxAngle = swingang;
      double newAngles[12] = {8.5*minAngle/9.,8.*minAngle/9.,6.*minAngle/9.,
      4.*minAngle/9.,2.*minAngle/9., 0.,
      2.*maxAngle/11.,4.*maxAngle/11.,6.*maxAngle/11.,
      8.*maxAngle/11.,10.*maxAngle/11., 10.5*maxAngle/11.};
      for(int i = 0; i<12;i++){
        targAngles[i] = newAngles[i];
      }
      //set_leg(targLeg[1]);
   ...
   /* double targAngles[12] = {minAngle,4.*minAngle/5.,3.*minAngle/5.,
    2.*minAngle/5.,minAngle/5., 0.,
    maxAngle/6.,2.*maxAngle/6.,3.*maxAngle/6.,
    4.*maxAngle/6.,5.*maxAngle/6., maxAngle};*/
    
    for(int i = 0; i<12; i++)
    {
        double targAngle = targAngles[i] - swingvel*10.;
        // - swingvel*15.;
